...
import { Alert, Button, Switch, Text, TextInput, View, Platform } from 'react-native';
...
export default function PrivacyControls() {
  ...
  return <View style={{ gap: 14, marginTop: 24, padding: 16, backgroundColor: colors.steel, borderRadius: 12 }}>
    ...
    {Platform.OS === 'web' && <><Text style={{ color: colors.white }}>Browser location works while the map is open. Background crossing alerts require the installed mobile app. Chrome and Codex have separate location permissions.</Text><Button title="Open live location map" onPress={() => router.push('/(tabs)/map')} /></>}
    {([['tracking', 'Background tracking'], ['alerts', 'Crossing alerts'], ['saveHistory', 'Save crossing history to my account']] as const).filter(([key]) => Platform.OS !== 'web' || key === 'saveHistory').map(([key,label]) => <View key={key} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}><Text style={{ color: colors.white, flex: 1 }}>{label}</Text><Switch accessibilityLabel={label} disabled={!ready || busy} value={prefs[key]} onValueChange={v => { void change(key,v); }} /></View>)}
    ...
    {Platform.OS !== 'web' && trackingError ? <Text style={{ color: colors.warning }}>{trackingError}</Text> : null}
