...
function StateSheet({
  ...
  const CARRY_SUMMARY: Record<CarryStatus, string> = {
    allowed:    'Reviewed guidance matches your profile. Check conditions and sources.',
    restricted: 'Reviewed restrictions apply to your profile.',
    prohibited: 'Reviewed guidance indicates a restriction for your profile.',
    unknown:    'Unable to determine. Your profile or reviewed rules may be incomplete.',
  };
        ...
        <Pressable style={sheet.container} onPress={() => {}}>
          ...
          <View style={[sheet.badge, { borderColor: statusColors[state.status] + '88', backgroundColor: statusColors[state.status] + '1A' }]}>
            ...
            <Text style={[sheet.badgeText, { color: statusColors[state.status] }]}>
              {state.status === 'allowed'    ? 'Reviewed guidance'       :
...
export default function MapScreen() {
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      const statuses = await getCarryStatusesForUser(STATES.map(s => s.code), permits, firearmsProfile);
      if (!cancelled) {
        setCarryMap(statuses);
        setLoadingMap(false);
      }
    }
  ...
  }, [permits, firearmsProfile]); // eslint-disable-line react-hooks/exhaustive-deps
