...
export default function LegalReferenceCard({ stateCode, compact = false }: { stateCode: string; compact?: boolean }) {
  ...
  return <View style={styles.card}>
    ...
    {stale ? <Text style={styles.warning}>Research notes need refreshing. Open the official sources for current information.</Text> : expanded && <>
    ...
    {compact && !stale && <Pressable accessibilityRole="button" accessibilityState={{ expanded }} onPress={() => setExpanded(v => !v)} style={{ paddingVertical: 10 }}><Text style={{ ...typography.body, color: colors.skyLight }}>{expanded ? 'Hide research details ↑' : 'Read research notes and open questions ↓'}</Text></Pressable>}
    {entry.sources.map(source => <Pressable key={source.url} accessibilityRole="link" accessibilityLabel={source.title} onPress={() => { setLinkError(''); void Linking.openURL(source.url).catch(() => setLinkError('Could not open the source. Check your connection and try again.')); }} style={styles.link}>
      ...
      <Text selectable style={styles.meta}>{compact ? new URL(source.url).hostname : source.url}</Text>
    </Pressable>)}
